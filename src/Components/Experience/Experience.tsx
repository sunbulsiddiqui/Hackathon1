...
import { ExperienceItem } from '../constant/Experience';  // Ensure the correct import path
import Image from 'next/image';
import React from 'react';

// Define the correct type for the ExperienceItem
interface ExperienceItemType {
  src: string;  // Image path
  label: string | number | React.ReactNode;  // Label can be string, number, or React Node
  value: string | number | React.ReactNode;  // Value can be string, number, or React Node
}
    ...
    <section className="relative py-10 sm:py-16 lg:py-20 bg-cover bg-center bg-[url('/menu.png')]">
      ...
      <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap justify-between items-center text-center">
          {ExperienceItem.map((item: ExperienceItemType, index: React.Key | null | undefined) => (
            <div 
              key={index} 
            ...
            >
              <div>
                <Image
                  src={item.src}  // Ensure that item.src is a valid path or import statement
